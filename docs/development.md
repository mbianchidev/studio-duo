...
ctest --test-dir build --build-config Release \
  --parallel 4 --output-on-failure --no-tests=error
```

CTest registers each suite independently so unrelated suites can run in
parallel. Reduce the parallel level on memory-constrained systems. The test
executable also accepts repeatable suite filters for focused work:
